...
const AuctionPage: React.FC = () => {
    ...
    const [isDisabled, setIsDisabled] = useState(false);
        const [winner, setWinner] = useState<{ name: string, profilePic: string } | null>(null);
            ...
            const fetchAuction = async () => {
                ...
                try {
                    ...
                    if (response.data.auction.mode === 'standard') {
                        ...
                        const start = new Date(response.data.auction.startingDate).getTime();
        ...
        useEffect(() => {
            ...
            const handleAuctionFinished = ({ auctionId, winner, winnerProfilePic }) => {
                if (auctionId === id) {
                    setTimeRemaining("EXPIRED");
                    setWinner({ name: winner, profilePic: winnerProfilePic });
                    setHighestBidderName(winner);
                    setHighestBidderPic(winnerProfilePic);
                }
            };
    
            on('bid-placed', handleBidPlaced);
            on('auction-finished', handleAuctionFinished);
    
            // Clean up the listener when the component unmounts
            return () => {
                off('bid-placed', handleBidPlaced);
                off('auction-finished', handleAuctionFinished);
            };
        }, [id, on, off]);
    
        // Timer Logic
        useEffect(() => {
            if (!auction || !auction.endingDate || timeRemaining === "EXPIRED") return;
                ...
                <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {timeRemaining === "EXPIRED" && winner ? (
                                 <div className="bg-white p-6 rounded-2xl border shadow-sm flex items-center gap-4">
                                    <Avatar className="h-10 w-10">
                                        <AvatarImage src={winner.profilePic} />
                    ...
                    <div className="bg-white p-6 rounded-3xl border shadow-sm h-fit">
                        <div className="flex items-center gap-2 mb-6 text-orange-600 font-bold">
                            <Gavel /> <span>{auction.mode === 'dutch' ? 'Buy Now' : 'Place Your Bid'}</span>
                        </div>
                        <div className="space-y-4">
                            ...
                            >
                                {timeRemaining === "EXPIRED" ? "Auction Ended" : auction.mode === 'dutch' ? 'Buy Now' : 'Confirm Bid'}
                            </button>
                        </div>
                    </div>
                </div>
